...
def ConsolidateData(ExcelFileList):
    ...
    for i in range (1,sheet_wb.nrows):
            ...
            if("Version mismatch" in sheet_wb.cell(i,4).value):
                    Script_Mismatch.append([sheet_wb.cell(i,0).value])
            if("Count Mismatch" in sheet_wb.cell(i,17).value ):
                    Test_Case_Mismatch.append([sheet_wb.cell(i,9).value])
            if("Version mismatch" in sheet_wb.cell(i,18).value ):
                    Syn_Result_TestScript.append([sheet_wb.cell(i,9).value])
            if("Time stamp is mismatching" in sheet_wb.cell(i,19).value ):
                    Time_Stamp_Mismatch.append([sheet_wb.cell(i,9).value])
...
def main():
                ...
                if len(MasterFileNameList)!= 0:
                        ...
                        count_mismatch_file.write("\nNo. Result files missing from synergy = "+str(Number_Result_Files_Not_present)+"/"+str(Total_script_Count))
...
def Help():
    ...
    lbl=Label(window1, text="Steps to execute the Synergy Analysis Tool:", fg='Black', font=("Times New Roman Bold", 16),anchor=CENTER,bg="light blue", padx=1000)
    lbl.pack(side="top")

##    lbl2=Label(window1, text="Steps to execute the Synergy Analysis Tool:",bd=2,fg='Black',anchor=CENTER,bg="light blue", font=("Arial Bold", 15))
##    lbl2.place(x=50,y=10)
    lbl3=Label(window1, text="1. Extract Main_App_SVCP from doors with the following attribute.",bd=2,fg='Black',font=("Times New Roman", 11))
    ...
    lbl3=Label(window1, text="3. Extract TPS SVCP surrogate from doors with following attribute.",bd=2,fg='Black',font=("Times New Roman", 11))
    ...
    lbl3=Label(window1, text="4. Select 3 input files in GUI.",bd=2,fg='Black',font=("Times New Roman", 11))
...
window.title("Synergy Analysis Tool" )
ratio = 16
...
lbl=Label(window, text="Synergy Analysis Tool", fg='Black', font=("Times New Roman Bold", 16),anchor=CENTER,bg="light blue", padx=1000)
lbl.pack(side="top")
